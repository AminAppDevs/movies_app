...
class MovieDetailsBloc extends Bloc<MovieDetailsEvent, MovieDetailsState> {
  ...
  MovieDetailsBloc(this.getMovieDetailsUsecase) : super(const MovieDetailsState()) {
    ...
    on<GetMovieDetailsEvent>((event, emit) async {
      ...
      result.fold((Failure failure) => emit(state.copyWith(movieDetailsState: RequestState.error, movieDetailsmessage: failure.message)),
          (MovieDetails movieDetails) => emit(state.copyWith(movieDetailsState: RequestState.loaded, movieDetails: movieDetails)));
    });

    /// get recommendation evebt
  }
}
